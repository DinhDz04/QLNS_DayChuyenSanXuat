...
import { layBanDoKhuVuc, luuBanDoKhuVuc } from "../services/khuVuc.service.js";

const GRID_SIZE = 12; // Bản đồ lưới 12x12

export default function BanDoKhuVuc() {
    const { id } = useParams();
    ...
    async function taiDuLieuBanDo() {
        setDangTai(true);
        setLoi("");
        setSelectedCdId(null);
        setSelectedDayChuyenId("");
        try {
            const res = await layBanDoKhuVuc(id);
            if (res.success) {
            ...
            setLoi(err.message || "Không thể tải sơ đồ bản đồ");
        } finally {
            setDangTai(false);
        }
    }
    ...
    // Đặt toàn bộ dây chuyền vào bản đồ
    function datDayChuyenVaoBanDo(dcId) {
        setCongDoanList(prevList => {
            ...
            // Chọn công đoạn đầu tiên của dây chuyền
    ...
    // Gỡ toàn bộ dây chuyền khỏi bản đồ
    ...
    // Thay đổi vị trí / xoay bằng nút bấm (có kiểm tra va chạm)
    ...
    // Nhấp vào ô trống trên lưới để gán tọa độ (có kiểm tra va chạm)
    function handleCellClick(x, y) {
        if (!laAdminOrLeaderKhuVuc) return;
        setLoi("");

        // Kiểm tra xem ô được click đã có công đoạn nào đè lên chưa (bao gồm cả ô span)
        ...
        if (occupiedCd) {
            ...
            // Di chuyển công đoạn đang chọn đến ô này
            setCongDoanList(prevList => 
                prevList.map(cd => {
    ...
    async function xuLyLuuBanDo() {
        ...
        // Kiểm tra chồng lấn lần cuối trước khi lưu
        ...
        try {
            ...
            const res = await luuBanDoKhuVuc(id, payload);
            if (res.success) {
    ...
    if (dangTai && !khuVuc) {
        return <div className="man-hinh-dang-tai">Đang tải bản đồ khu vực...</div>;
    }
    ...
    if (loi && !khuVuc) {
        return (
            <div className="noi-dung-admin">
                ...
                <button className="nut-chinh" onClick={() => navigate("/admin/khu-vuc")}>Quay lại</button>
            </div>
        );
    }
    ...
    // Lọc danh sách theo bộ lọc Dây chuyền được chọn
        ...
        <div className="noi-dung-admin" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 120px)" }}>
            <div className="admin-header-bar" style={{ flexShrink: 0, borderBottom: "1px solid #cbd5e1", paddingBottom: "12px", marginBottom: "16px" }}>
                <div className="tieu-de-khoi">
                    <h2>Bản đồ mặt bằng: {khuVuc?.ten_khu_vuc}</h2>
                ...
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                        <span style={{ fontSize: "13px", fontWeight: "bold", color: "var(--charcoal)" }}>Lọc Dây chuyền:</span>
                        <select 
                    ...
                    <button className="nut-chinh" onClick={() => navigate("/admin/khu-vuc")}>
                        Quay lại
                    </button>
                </div>
            </div>
                ...
                <div style={{ width: "300px", display: "flex", flexDirection: "column", gap: "16px", flexShrink: 0, overflowY: "auto" }}>
                    ...
                    {selectedStep ? (
                        ...
                        <div className="the-thong-tin" style={{ padding: "16px", margin: 0, fontStyle: "italic", fontSize: "12px", color: "var(--text-muted)" }}>
                            💡 Click **"+ Đặt sa bàn"** trên thẻ Dây chuyền chưa đặt ở trên để đưa toàn bộ các công đoạn của Line lên sa bàn lưới, sau đó di chuyển các công đoạn bằng cách nhấp ô trống.
                        </div>
                    )}
                </div>
